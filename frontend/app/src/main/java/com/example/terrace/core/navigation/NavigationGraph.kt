    ...
    fun NavigationGraph(navController: NavHostController, usageViewModel: UsageViewModel) {
        ...
        NavHost(navController = navController, startDestination = Screen.Loader.route) {
            ...
            composable("home/{score}/{isFriend}") { backStackEntry ->
                val score = backStackEntry.arguments?.getString("score")?.toIntOrNull() ?: 0
                val isFriend = backStackEntry.arguments?.getString("isFriend")?.toBooleanStrictOrNull() ?: false
                HomeScreen(navController, usageViewModel, isFriend, score)
            }
